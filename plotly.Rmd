---
title: 'Developing Data Products: Cryptocurrencies'
author: "Park"
date: "11/9/2020"
...
This is the second assignment of the *Developing Data Products*, the fourth course in the *Statistics and Machine Learning* Specialization in Coursera.

The project attempts to plot the daily open prices of 5 cryptocurrencies from *28 April 2013* to *29 November 2018* using **plotly**. The 5 cryptocurrencies are the most expensive ones based on the price on **10 September 2020** which are also available on the data set. Please give the credit to *jvent* from [Kaggle](https://www.kaggle.com/jessevent/all-crypto-currencies) who provides this free data set.

## Historical Price Chart
For the code, please see the following slides.
...
```{r plotcode1, echo = TRUE, eval = FALSE}
# load the data
data <- read.csv("crypto-markets.csv")
...
## Code for the Plot (Continued)

```{r plotcode2, echo = TRUE, eval = FALSE}
# create a list of data frames, each contains one of the top 5 
# currencies' data
df <- list()
for (i in 1:length(crypto)) {
...
## Code for the Plot (Continued)
``` {r plotcode3, echo = TRUE, eval = FALSE}
# combine all into 1 data frame
# Bitcoin (first item in the list) has the longest history
topfive <- data.frame(Date = df[[1]]$Date,  
                      Bitcoin = NA,
                      Ethereum = NA,
                      Bitcoin.Cash = NA,
...
## Code for the Plot (Continued)
```{r plotcode4, echo = TRUE, eval = FALSE}
library(plotly)
plot_ly(topfive, x = ~Date, y = ~Bitcoin, type = "scatter", mode = 
            "lines", name = "Bitcoin") %>%
        add_trace(y = ~Ethereum, name = "Ethereum") %>%
        ...
        layout(xaxis = list(title = "Date"),
            yaxis = list(title = "Daily Open Prices (USD)"),
            title = "Daily Open Prices for the 5 Most Expensive 
            Cryptocurrencies")
```
